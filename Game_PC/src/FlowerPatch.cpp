...
FlowerPatchRef FlowerPatch::create()
{
    return  make_shared<FlowerPatch>();
}
void FlowerPatch::addFlower(float x,float y,float xL,float yL,float size2,ci::Surface &gradient)
{
    Perlin perlin;
    perlin.setSeed(20);
   ...
   // ci::ColorA grassColor =  gradient.getPixel(vec2(noise*255,0));
    ...
    texCoord0.push_back(vec3(rot.x,rot.y,-glm::linearRand(2.0, 5.0)));
...
void FlowerPatch::updatePlayer(ci::vec2 playerPos)
{
