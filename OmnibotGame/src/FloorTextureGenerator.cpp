...
void FloorTextureGenerator::setup()
{
	try{
		...
		map = gl::Texture::create(loadImage(loadAsset("floor/map.png")));
		generated = true;
	}
	catch(...)
	{
		
		size = GSettings::worldSize;
		mFbo = gl::Fbo::create( size, size );
		map = mFbo->getColorTexture();
...
void FloorTextureGenerator::generate(){
   ...
   std::vector<TriVec> vertices;
	...
	writeImage(getAssetPath("").string() + "/floor/map.png", mFbo->getColorTexture()->createSource());
	gl::viewport(0, 0, GSettings::windowWidth, GSettings::windowHeight);
}
